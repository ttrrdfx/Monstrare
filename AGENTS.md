...
若任務已由看板追蹤，實作開始、驗證完成及交付前都要核對本卡的
`stage`、`agent`、`links.verificationReport` 與 `evidence`，依實際進度更新。
任務卡寫「不得觸碰 cards/epics」時，預設保護其他卡片與 Epic 資料，
不排除本卡的進度欄位；若明確禁止更新本卡，交付時須說明看板未同步的原因。
Code review 或人工驗收尚未完成時，不得只因測試通過就標成 `done`。
